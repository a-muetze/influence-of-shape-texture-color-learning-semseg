...
## added RGB2HS
@TRANSFORMS.register_module()
class RGB2HS(BaseTransform):
    """Convert RGB image to HS image.
    ...
    def transform(self, results: dict) -> dict:
        """Call function to convert RGB image to hs image.

        Args:
            results (dict): Result dict from loading pipeline.

        Returns:
            dict: Result dict with hs image.
        """
        bgr = results['img']
        ...
        if self.out_channels != 2:
            exit("HS has exactly two channels")
        else:
            img = hs
